...
from txomon.ticket.model import Milestone, group_milestones, Ticket
...
class QueryModule(Component):
    ...
    def get_supported_conversions(self):
        yield ('rss', _('RSS Feed'), 'xml',
               'txomon.ticket.Query', 'application/rss+xml', 8)
        yield ('csv', _('Comma-delimited Text'), 'csv',
               'txomon.ticket.Query', 'text/csv', 8)
        yield ('tab', _('Tab-delimited Text'), 'tsv',
               'txomon.ticket.Query', 'text/tab-separated-values', 8)
    ...
    def get_navigation_items(self, req):
        from txomon.ticket.report import ReportModule
    ...
    def process_request(self, req):
        ...
        if format:
            ...
            Mimeview(self.env).send_converted(req, 'txomon.ticket.Query', query,
...
class TicketQueryMacro(WikiMacroBase):
    ...
    def expand_macro(self, formatter, name, content):
        ...
        if format == 'progress':
            from txomon.ticket.roadmap import (RoadmapModule,
